...
- `src/` — código-fonte da aplicação:
  ...
  - `paginas/` — os ecrãs, cada um com o seu `.module.css` ao lado:
    ...
    - `Inicio.tsx` — a página de análise (`/`), com um desenho deliberadamente diferente do resto da app: sem título de página, Saldo Total num cartão, e a repartição de Entradas/Saídas por categoria (`GET /resumo`) atrás de um controlo segmentado — ver "Estado actual".
...
**Início** (`/`) — a página de ANÁLISE, ao contrário de Contas (gestão pura) e Movimentos (o extrato), e a única com um desenho deliberadamente diferente do resto da app (é a página de destino, e a única de análise): sem título de página, por agora. O Saldo Total vive num cartão de fundo destacado (soma do saldo actual de todas as contas, convertido para a moeda principal — não depende de nenhum período); por baixo, o mês atual por título (ex.: "Setembro 2026", vindo do próprio backend) com o Líquido do mês ao lado, e um CONTROLO SEGMENTADO "Entradas"/"Saídas" (à maneira do iOS) — só a repartição por categoria da direcção escolhida fica visível de cada vez (o total dessa direcção em destaque, as suas categorias como barras horizontais por baixo, "Ver mais" além das 5 maiores). A primeira versão mostrava as duas direcções sempre lado a lado, com uma barra comparativa extra — trocada por esta, mais simples, depois de a duplicação de informação (o verde/vermelho a repetir-se em três sítios) ter sido sentida como confusa. Cor mantém-se conservadora — só as duas cores semânticas já estabelecidas (positivo/negativo), sem nenhum acento novo. Tocar numa barra de categoria abre/fecha, ali mesmo, a sua repartição por SUBCATEGORIA (`GET /resumo/categorias/{grupo_id}`, só pedida na primeira vez que esse grupo é aberto — as respostas ficam em memória, reabrir não volta a pedir); só um grupo aberto de cada vez, e trocar de direção ou de "Ver mais"/"Ver menos" fecha qualquer um que estivesse aberto. A percentagem de cada subcategoria é face ao total DO GRUPO, não ao total geral. Ainda sem filtros (contas, categorias, período à escolha), que ficam para uma fatia futura.
